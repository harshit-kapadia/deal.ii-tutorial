...
void laplace<dim>::solve_current_time (double dt)
{
  ...
  local_system_rhs *= (dt) ;
  local_basis_inner_product.vmult(temporary, solution) ;
...
void laplace<dim>::output_results ()
{
  ...
  for (; cell!=endc; ++cell)
  {
    ...
    VectorTools::point_value	(dof_handler, solution, center, solution_value);
